...
def gaussian_blur(img, ksize=5, sigma=1.0):
    if ksize == 0:
        # MODIFIED: Auto-calculate kernel size to ensure enough coverage for the sigma
...
def build_gaussian_pyramid(img, num_octaves=4, num_scales=5):
    ...
    for o in range(num_octaves):
        ...
        # MODIFIED: Use INTER_LINEAR for downsampling to maintain better feature quality
        h, w = img.shape
...
def detect_keypoints(dog_pyramid, threshold=0.03):
    ...
    for o, octave in enumerate(dog_pyramid):
        for s in range(1, len(octave) - 1):
            ...
            # MODIFIED: Optimized neighborhood search to reduce redundant calculations
...
def assign_orientation(magnitude, orientation, kp, scale, num_bins=36, window_factor=1.5):
    o, s, i, j = kp
    h, w = magnitude.shape
    # MODIFIED: Window size adjusted to 3 * sigma for standard SIFT orientation stability
...
def compute_descriptor(magnitude, orientation, kp, dominant_angle, scale, num_spatial_bins=4, num_orient_bins=8):
    ...
    # MODIFIED: Standardized window size to 16x16 for a consistent 128-D vector
    window_size = 16 
    ...
    for di in range(-half_w, half_w):
        for dj in range(-half_w, half_w):
            ...
            if 0 <= ni < h and 0 <= nj < w:
                ...
                # MODIFIED: Map relative pixel position to one of the 4x4 spatial bins
                ...
                if 0 <= bin_i < num_spatial_bins and 0 <= bin_j < num_spatial_bins:
                    ...
                    # MODIFIED: Apply Gaussian weight to prioritize center samples
                    weight = np.exp(-(di**2 + dj**2) / (2 * (half_w)**2))
                    descriptor[bin_i, bin_j, orient_bin] += magnitude[ni, nj] * weight

    desc = descriptor.flatten()
    # MODIFIED: Normalize, clip at 0.2, and re-normalize to ensure illumination robustness
...
def extract_descriptors(keypoints, gaussian_pyramid, sigma0=1.6, num_scales=5):
    start = time.time()
    k = 2 ** (1.0 / (num_scales - 1))
    descriptors = []
    valid_keypoints = []
...
def run_sift_pipeline(image, num_octaves=4):
    ...
    descriptors, valid_kps, desc_time = extract_descriptors(keypoints, gauss_pyr)
    ...
    # print(f"Detection Time: {t_detect_end - t_detect_start:.4f}s")
